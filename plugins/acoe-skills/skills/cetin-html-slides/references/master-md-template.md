...
## Slide switches (add this block near the top of the file)

```markdown
**Slide switches:** every built HTML file opens with a `window.SLIDE_SWITCHES` block in `<head>`.
Flip any value to `false` and that slide disappears completely — deck, numbering, arrows, the
chapter divider's agenda list, and the contents page. Keys are the slide IDs (`2.7`), plus
`title` and `topic-1`…`topic-N`. Regenerated by `scripts/slide_switches.py` after every build;
defaults live in that script so a rebuild does not switch things back on.
```

## Companion file

A deck with more than one presenter or a timed agenda also gets `<Name>_storyline.md` —
presenter cue cards, clock timings, the line that lands per demo, and an objection table.
See `references/storyline-template.md`. Renumbering a deck means updating **five** places:
slide ids, the divider agenda, the contents page, this file and the storyline.
