...
`cmp_FilterButton.Choices` and `cmp_Navigation.Screens` both shipped a Default
that could not establish a concrete table schema (blank, and a bare app-scope
name reference respectively). The Studio silently falls back to a placeholder
schema (`SampleBooleanField`/`SampleNumberField`/`SampleStringField`), and a
caller's real table then fails to type-match it — 7 of the run's errors, on
top of the identical defect already fixed once on `cmp_FieldChoice.Choices`
in the prior round. Three components, two rounds: this test file exists so a
fourth occurrence fails locally instead of live.
"""
import pathlib
import sys
import unittest
...
class TestBadTableDefaultsAreCaught(unittest.TestCase):
    """The two shapes confirmed live: blank, and a bare name reference."""
    ...
    def test_bare_app_scope_name_reference_is_an_error(self):
        ...
        findings = errors(ccp.find_bad_table_defaults(text, "f.pa.yaml"))
        self.assertEqual(len(findings), 1)
        self.assertEqual(findings[0].prop, "Screens")

    def test_dotted_bare_reference_is_also_an_error(self):
